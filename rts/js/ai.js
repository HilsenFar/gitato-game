...
RTS.ai = (() => {
  ...
  function create(player, diff) {
    return {
      p: player,
      diff: diff || 'normal',
      d: RTS.AI_DIFF[diff] || RTS.AI_DIFF.normal,  // difficulty parameters (const.js)
      wave: (RTS.AI_DIFF[diff] || RTS.AI_DIFF.normal).wave0, // army size that triggers the next attack
      lastDecision: 0,
      thinkN: 0,        // think counter (easy runs economy every 2nd think)
  ...
  function think(ai, s) {
    ...
    const cmds = [];
    const p = ai.p;
    const d = ai.d || RTS.AI_DIFF.normal;
    ai.thinkN = (ai.thinkN || 0) + 1;
    const eco = ai.thinkN % d.ecoEvery === 0;  // economy/production decisions this think?
    ...
    } else if (eco && !(d.expand && tryExpand(s, ai, cmds, hqs, raxes, workers, res, my('hq')))) {
      if (!wantBuild('rax', raxes.length === 0)) {
        if (!wantBuild('turret', raxes.length > 0 && turrets.length < d.turretCap && workers.length >= 6 && res >= 250)) {
    ...
    // --- upgrades (hard only): Mining Drill first, then Combat Stims ---
    if (d.tech && eco) {
      const TT = RTS.TECH;
      if (!(s.tech[p] & TT.drill.bit)) {
        if (res >= TT.drill.cost + 150) cmds.push({ p, c: 'upgrade', tid: hq.id, tech: 'drill' });
      } else if (!(s.tech[p] & TT.stims.bit)) {
        const rax = built('rax')[0];
        if (rax && res >= TT.stims.cost + 400) cmds.push({ p, c: 'upgrade', tid: rax.id, tech: 'stims' });
      }
    }
    ...
    // armyCap counts queued combat units too, or the cap leaks: queues keep
    // draining while new orders are issued and easy fields ~2x its cap
    const queuedArmy = mine.reduce((n, b) =>
      n + (b.queue ? b.queue.filter((q) => q.kind !== 'worker').length : 0), 0);
    const wantArmy = !d.armyCap || army.length + queuedArmy < d.armyCap; // easy fields a small army
    if (eco && wantArmy) {
    ...
    // (hard aborts even a committed push to save its base: d.recall)
    const attacked = mine.find((e) => e.lastHitTick && s.tick - e.lastHitTick < 60 && K[e.kind].bld);
    if (attacked && (d.recall || s.tick > ai.attackUntil)) {
      if (d.recall) ai.attackUntil = s.tick; // recommit after the defense
    ...
    if (d.harassEvery > 0 && facts.some((f) => f.prog >= 1) && raiders.length >= 2 && s.tick >= ai.nextHarass) {
      ...
      ai.nextHarass = s.tick + 20 * d.harassEvery; // squad frequency set by difficulty
    }
    ...
    if (s.tick >= 20 * d.firstAttack &&
        (army.length >= ai.wave || overdue) && s.tick > ai.attackUntil) {
      ...
      ai.wave = Math.min(d.waveCap, ai.wave + 4);
      ai.attackUntil = s.tick + 20 * d.waveCommit; // commit time / pause between waves
    }
